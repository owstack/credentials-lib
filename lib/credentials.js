...
var mnemonicLib = require('@owstack/mnemonic-lib');
...
function Credentials(opts) {
  ...
  this.account = 0;
  this.currency = opts.currency || Networks.defaultNetwork.symbol;
};
...
Credentials.fromRandom = function(opts) {
  opts = opts || {};
  return fromRandom(
    opts.network || Networks.defaultNetwork.symbol);
};
...
Credentials.prototype.addWalletInfo = function(walletId, walletName, m, n, copayerName) {
  ...
  this.copayerId = xPubToCopayerId(this.xPubKey);
...
function fromExtendedPrivateKey(xPrivKey, account, derivationStrategy, opts) {
  ...
  var x = new Credentials(); // TODO: default network will be set
  x.xPrivKey = xPrivKey;
  x.account = account;
...
function fromExtendedPublicKey(xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  var x = new Credentials(); // TODO: default network will bet set
  x.xPubKey = xPubKey;
  x.entropySource = owsCommon.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
function getNetworkFromExtendedKey(xKey) {
  ...
  return Networks.get(xKey).symbol;
};

function xPubToCopayerId(xpub) {
...
function hashFromEntropy(entropySource, prefix, length) {
  ...
  var b2 = owsCommon.Hash.sha256hmac(b, new Buffer(prefix));
...
function expand(credentials) {
  ...
  if (credentials.entropySourcePath) {
    ...
    credentials.entropySource = owsCommon.Hash.sha256sha256(seed).toString('hex');
  }
  ...
  if (credentials.entropySource) {
    ...
    credentials.entropySource = owsCommon.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
