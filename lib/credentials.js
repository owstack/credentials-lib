...
function fromExtendedPublicKey(xPubKey, source, entropySourceHex, account, derivationStrategy, opts) {
  ...
  x.xPubKey = xPubKey;
  x.entropySource = keyLib.crypto.Hash.sha256sha256(entropyBuffer).toString('hex');
  x.account = account;
  x.derivationStrategy = derivationStrategy;
...
function hashFromEntropy(entropySource, prefix, length) {
  ...
  var b2 = keyLib.crypto.Hash.sha256hmac(b, new Buffer(prefix));
...
function expand(credentials) {
  ...
  if (credentials.entropySourcePath) {
    ...
    credentials.entropySource = keyLib.crypto.Hash.sha256sha256(seed).toString('hex');
  }
...
//    var requestDerivation = deriveFn(Constants.PATHS.REQUEST_KEY);
    ...
    credentials.entropySource = keyLib.crypto.Hash.sha256(requestDerivation.privateKey.toBuffer()).toString('hex');
  }
