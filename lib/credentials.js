...
Credentials.prototype.extend = function(info) {
  ...
  Credentials.extendedFields[this.type] = lodash.uniq(lodash.difference(lodash.concat(Credentials.extendedFields[this.type], Object.keys(info)), FIELDS));
  this.extendedFields = Credentials.extendedFields[this.type];
};
...
Credentials.prototype.isPrivKeyExternal = function() {
  return this.hasExternalSource();
};
...
Credentials.prototype.getNetworkFromExtendedKey = function(xKey) {
  xKey = xKey || this.xPrivKey || this.xPubKey;
...
function expand(credentials) {
  ...
  var network = credentials.getNetworkFromExtendedKey(credentials.xPrivKey || credentials.xPubKey);
  var networkName;
  if (network) {
    networkName = network.name;
  }
