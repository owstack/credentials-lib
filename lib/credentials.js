...
function Credentials(opts) {
  ...
  this.account = 0;
  this.currency = opts.currency || (opts.network ? Networks.get(opts.network).preference : undefined) || Networks.defaultNetwork.preference;
  this.network = (opts.network ? opts.network.code : undefined) || Networks.defaultNetwork.code;
};
 ...
 */
function fromMnemonic(words, network, passphrase, account, derivationStrategy, opts) {
  checkNetwork(network);
  $.shouldBeNumber(account);
  $.checkArgument(lodash.includes(lodash.values(Constants.DERIVATION_STRATEGIES), derivationStrategy));
...
function getNetworkFromExtendedKey(xKey) {
  ...
  var version = BufferUtil.integerFromBuffer(data.slice(0, 4));
  var n = Networks.get(version, ['version.xprivkey', 'version.xpubkey']);
  return n.code;
};
...
function expand(credentials) {
  ...
  credentials.copayerId = xPubToCopayerId(credentials.xPubKey);
